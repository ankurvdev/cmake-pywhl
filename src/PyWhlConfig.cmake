...
define_property(
  GLOBAL
  PROPERTY PYWHL_MODULE_MANIFESTS
  INHERITED
  BRIEF_DOCS "PYWHL Module Manifests"
  FULL_DOCS
    "This property is used to track PYWHL modules available for packaging")

define_property(
  GLOBAL
  PROPERTY PYWHL_TARGET_DEPENDENCIES
  INHERITED
  BRIEF_DOCS "PYWHL Package Target dependencies"
  FULL_DOCS
    "This property is used to track the target dependencies of PYWHL packages")

function(add_pywhl_module name)
  cmake_parse_arguments("args" "" "EXCLUDE_REGEX" "DATA;SCRIPTS;TARGETS"
                        ${ARGN})
  ...
  # Prefix with $<CONFIG> so multi-config generators (e.g. Visual Studio) get a
  # distinct output file per configuration. The content embeds
  # $<TARGET_FILE:...>, which differs per config; without a per-config output
  # name cmake errors with "Evaluation file to be written multiple times with
  # different content".
  set(manifest_file "${CMAKE_CURRENT_BINARY_DIR}/$<CONFIG>_${name}.ini")
  file(
    GENERATE
    OUTPUT "${manifest_file}"
    CONTENT "${contents_package_ini}")

  set_property(GLOBAL APPEND PROPERTY PYWHL_MODULE_MANIFESTS
                                      "${name}:${manifest_file}")
  foreach(tgt ${args_TARGETS})
    set_property(GLOBAL APPEND PROPERTY PYWHL_TARGET_DEPENDENCIES
                                        "${name}:${tgt}")
  endforeach()
endfunction()

function(_pywhl_generate_whl_name)
  cmake_parse_arguments("args" "" "OUTPUT_VAR;DISTRIBUTION;VERSION;BUILD;" ""
                        ${ARGN})
  # The wheel filename is {distribution}-{version}(-{build tag})?-{python
  # tag}-{abi tag}-{platform tag}.whl. PIP-PYWHLs are both dogmatic about
  # version string formats, but accept sem-ver 2 compliant version-strings. The
  # PYWHL filename, however, needs to encode all `-` as `_` in the
  # version-string.
  ...
  elseif(UNIX)
    ...
    message(
      FATAL_ERROR
        "add_pywhl_package: Unsupported platform for wheel: ${CMAKE_SYSTEM_NAME}"
    )
  endif()
  set(extension whl)

  set(whl_name
      "${args_DISTRIBUTION}-${version}${build}-${python_tag}-${abi_tag}-${platform_tag}.${extension}"
  )
  set(${args_OUTPUT_VAR}
      "${whl_name}"
      PARENT_SCOPE)
...
function(add_pywhl_package targetName)
  ...
  cmake_parse_arguments(
    "args"
    ""
    "WHL_FILE_OUTVAR;DEV_TARGET;EDITABLE_TARGET;VERSION;LICENSE_FILE;METADATA_FILE;ENTRY_POINTS_FILE"
    "MODULES"
    ${ARGN})

  if(NOT DEFINED args_DEV_TARGET)
    set(args_DEV_TARGET "${targetName}_dev_whl")
  endif()

  if(NOT DEFINED args_EDITABLE_TARGET)
    set(args_EDITABLE_TARGET "${targetName}_editable_whl")
  endif()

  if(NOT DEFINED args_VERSION)
    set(args_VERSION "0.0.1")
    message(
      WARNING "add_pywhl_package: version not specified. Using VERSION=0.0.1")
  endif()

  if(NOT DEFINED args_LICENSE_FILE AND EXISTS
  ...
  if(NOT DEFINED args_METADATA_FILE AND EXISTS
  ...
  if(NOT DEFINED args_METADATA_FILE)
    message(
      WARNING
        "Metadata file not found for pywhl package: ${targetName}. Please create one at ${CMAKE_CURRENT_SOURCE_DIR}/METADATA"
    )
  endif()

  if(NOT DEFINED args_ENTRY_POINTS_FILE
     AND EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/entry_points.txt")
  ...
  foreach(modname ${args_MODULES})
    ...
    if("${modmanifest}" STREQUAL "NOTFOUND")
      message(
        FATAL_ERROR
          "add_pywhl_package: pywhl module ${modname} not found for package ${targetName}"
      )
    endif()
  ...
  _pywhl_generate_whl_name(DISTRIBUTION ${targetName} VERSION ${args_VERSION}
                           OUTPUT_VAR whl_name)

  set(editable_whl_file
      "${build_dir}/${targetName}-${args_VERSION}-0.editable-py3-none-any.whl")
  ...
  file(
    ...
    OUTPUT "${manifest_file}"
    CONTENT "${build_ini}")
  set(dev_depfile "${manifest_file}.dev.d")
  set(editable_depfile "${manifest_file}.editable.d")
  set(package_depfile "${manifest_file}.package.d")

  add_custom_command(
    COMMAND
      "${Python3_EXECUTABLE}" "${PYWHL_SCRIPT_FILE_PATH}" --verbose --manifest
      "${manifest_file}" --dep-file "${package_depfile}" --work-dir
  ...
  add_custom_command(
    COMMAND
      "${Python3_EXECUTABLE}" "${PYWHL_SCRIPT_FILE_PATH}" --verbose --manifest
      "${manifest_file}" --dep-file "${dev_depfile}" --work-dir "${build_dir}"
  ...
  add_custom_target(
    ${args_DEV_TARGET} ALL DEPENDS "${dev_depfile}" "${PYWHL_SCRIPT_FILE_PATH}"
                                   "${manifest_file}")

  add_custom_target(
    ${args_EDITABLE_TARGET} ALL
  ...
  foreach(pkgname ${args_MODULES})
    foreach(dependency ${dependencies})
      ...
      if("${pkgname}" STREQUAL "${pkgname2}")
        ...
        add_dependencies(${args_DEV_TARGET} ${deptarget})
      endif()
    endforeach()
  endforeach()
  ...
  if(NOT TARGET all_dev_whl)
    add_custom_target(all_dev_whl)
  endif()
  if(NOT TARGET all_editable_whl)
    add_custom_target(all_editable_whl)
  endif()

  add_dependencies(all_dev_whl ${args_DEV_TARGET})
  add_dependencies(all_whl ${targetName} ${args_DEV_TARGET})
  add_dependencies(all_editable_whl ${args_EDITABLE_TARGET} ${args_DEV_TARGET})

  if(DEFINED args_WHL_FILE_OUTVAR)
    set(${args_WHL_FILE_OUTVAR}
        "${whl_file}"
        PARENT_SCOPE)
  endif()
endfunction()
...
macro(PyWhl_Init)
  ...
  if(NOT DEFINED PYWHL_SCRIPT_FILE_PATH AND (DEFINED PYWHL_CONTENTS_PYWHL_PY))
    ...
    if(NOT DEFINED PYWHL_CONTENTS_EDITABLE_FINDER_PY)
      message(
        FATAL_ERROR
          "Unable to generate ${CMAKE_CURRENT_BINARY_DIR}/editable_finder.py")
    endif()
    file(
      GENERATE
      OUTPUT "${PYWHL_SCRIPT_FILE_PATH}"
      CONTENT "${PYWHL_CONTENTS_PYWHL_PY}")
    file(
      GENERATE
      OUTPUT "${CMAKE_CURRENT_BINARY_DIR}/editable_finder.py"
      CONTENT "${PYWHL_CONTENTS_EDITABLE_FINDER_PY}")
  endif()
endmacro()
